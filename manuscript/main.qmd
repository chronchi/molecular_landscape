...
carcinogenesis. It drives different subtypes in breast cancer and
treatment decisions. In most of the cases, if a patient is diagnosed
with breast cancer and has immunohistochemistry (IHC) percentage of 1% for
estrogen receptor, they will receive standard endocrine therapy. We show
here that estrogen signaling is a continuous status rather than
dichotomous. To further elucidate the effects of estrogen signaling and
other molecular pathways, we propose a new framework for personalized
medicine. First, we introduce a novel embedding technique based on
...
(PAM50). We show that the embedding
is not dependent of tumor purity, age and other clinical variables and
other pathways, such as G2M Checkpoint, Epithelial to Mesenchymal Transition
(EMT) and PI3K AKT MTOR signaling drives the embedding. 
...
be possible reasons that explain the response to treatment. Lastly, 
...
Breast cancer is the most commonly diagnose tumor disease in the world [@whocancer].
It is also a very heterogeneous disease that can be subdivided in 
different subtypes. Patients that are diagnosed with 
breast cancer are classified based on the expression of the estrogen,
progesterone receptors and HER2 membrane protein. The 
estrogen receptor positive (ER+) breast cancer subtype is the most
frequent found in clinical practice [@Anderson2002]. 
...
can range from 1% to 100% according to ASCO guidelines [@Allison2020]. It has been
...
Transcriptomics and genomics have revolutionazed how cancer research is done. 
The new related
...
aromatase inhibitors. Several of these signatures are already available
to be used in hospitals [@mammaprint; @pam50; @oncotypedx; @endopredict; @bci].
Moreover,
...
related to estrogen, proliferation, HER2 and invasion. In some of the cases
the risk score is more strongly associated with the estrogen module and in
other cases to the proliferation module [@Buus2021]. There is still a lack of
...
of the signaling pathways in a patient level. By developing a normalization
...
[@tcgabrca; @Saal2015; @Curtis2012; @Gao2019], and that it is possible
...
[@Subramanian2005; @Liberzon2011] to understand
the biology of the tumors in patient and neighborhood levels. 
...
where k corresopnds to the number of cohorts, in this case $k=2$.
...
downstream analysis. PCA was used with centering and scaling set to 
`FALSE`. The embedding for new individual samples is obtained
...
stage and number of lymph nodes for TCGA and SCANB. For METABRIC the
...
Estrogen signaling is an indicative of how well a patient will respond
to endocrine therapy. There are several ways to represent the estrogen
signaling, one of them is to calculate scores derived from gene
expression levels of bulk RNA-sequencing.
...
Cox regression was used to determine the hazard ratio of the 
estrogen signaling
...
when possible only those that received endocrine therapy. The selected
sub population represents patients that could benefit from 
additional adjuvant therapies given unfavorable scores.
...
This shows the continuous aspect of estrogen receptor status. For both
estrogen pathways of the hallmark collection of gene sets, hazard ratios
were below 1 in all cases (Table 1, supplementary material), with a 
...
levels between ER+ and ER- breast cancer samples. Examples of genes 
where see such differences are TFF1 and ESR1 (Figure S1 (b)).
...
integrated. All samples, including those used for training
...
(@fig-02 (e)). Another problem inherent to publicly available 
...
missing from the dataset is key for the embedding (@fig-02 (f)). The
higher the proportion the less precise the embedding is. Other clinical
factors, such as tumor stage, node stage, age, NPI and tumor purity
show no influence in the embedding (Figure S2), besides the fact
that ER- BC patients usually have higher stage in general.
...
METABRIC. It is also in between the two cohorts. We also show
that the embedding works for the SMC cohort [@Kan2018] 
(supplementary figure 3; 3.14 from the book).
...
The POETIC trial team sent for microarray hybridization
...
patients indeed have different molecular biological properties. 
...
There are several driving the distinction between samples in the 
...
only endocrine therapy (METABRIC: OS/RFS and SCANB: OS) and have 
$PC3 > 0$ show that G2M 
...
aditional adjuvant therapy targeting this specific pathway.
