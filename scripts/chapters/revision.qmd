...
avg_rankings %>% 
    ...
    ggplot2::labs(
        x = "Average ranking of stable genes",
        y = "ER IHC"
    ) + 
    ggplot2::facet_wrap(~cohort, scales = "free", ncol = 1) +
...
p + 
    ...
    ggplot2::labs(
        title = "Black dots correspond to METS PROJECT samples",
        color = "Molecular\nsubtype"
    ) +
    ggplot2::scale_color_manual(
        values = get_colors_pam50(df_pca),
...
df_pca %>% 
    ...
    ggplot2::labs(
        title = paste0("LISI score: ", round(mean(lisi_scores_scanb[, 1]), digits = 2)),
        subtitle = paste0("Expected maximum LISI score: ", unname(expected_ilisi_vals))
    ) + 
    ggplot2::theme_bw(base_size = 20) + 
    ggplot2::theme(legend.position = "none")
```
