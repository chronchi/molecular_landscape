...
plots_poetic$cohort <- plots_poetic$cohort +
    ...
    #) + 
    ggplot2::scale_color_manual(
        labels = c("metabric" = "METABRIC", "tcga" = "TCGA",
                   "scanb" = "SCAN-B", "poetic" = "POETIC"),
        values = scales::hue_pal()(4)
    ) +
    ggplot2::labs(title = NULL, subtitle = NULL, color = "Cohort") +
    ggplot2::theme_bw(base_size = 20)

plot_list$cohorts <- plots_poetic$cohort
...
plot_responder_poetic <- poetic_df_pca_baseline %>%
    ...
    ggplot2::scale_color_manual(
        ...
        limits = c("Non responder", "Not available", "Responder")
    ) +
...
plot_list$is_responder <- plot_responder_poetic 
...
plot_list$diff_pc3_poetic <- poetic_df_pca_baseline %>%
...
plot_list$diff_pc3_poetic_bayes <- pc3_responders_comparison_df %>%
...
plot_list$diff_pc3_poetic_bayes
```
...
pc3_responders_comparison_df %>%
    ...
    tidybayes::stat_dotsinterval(quantiles = 100, .width = c(.66, .95)) + 
    ggplot2::labs(
        x = "Expectation of the posterior predictive distribution",
        y = NULL
    ) +
    ggplot2::theme_bw(base_size = 20) + 
    change_plot_aes_point() + 
...
#| fig-cap: Differences of the embeddings for each patient individually
#|     stratified by response status. Each dot corresponds to a single patient.
plot_list$differenc_responder_non_responder <- means %>%
    dplyr::mutate(
        is_responder = c("non_responder" = "Non responder", "responder" = "Responder")[is_responder]
    ) %>%
    ggplot2::ggplot(aes(x = .epred, y = is_responder)) +
...
plot_list$differenc_responder_non_responder
```
...
plot_list$plot_patients <- plot_patients
...
plot_list$is_responder_middle <- plots_poetic_sub$is_responder +
    ggplot2::labs(title = NULL, subtitle = NULL, color = "Is responder?") +
    ggplot2::scale_color_manual(values = c(
        "Responder" = scales::hue_pal()(3)[3],
        "Non responder" = scales::hue_pal()(3)[1]
    )) +
    ggplot2::scale_size_manual(
        values = c(.1, 3, .1, .1),
        guide = "none"
...
Below we compiled some of the plots for a figure in the paper, so they look
nicer together.

```{r, fig.width=22, fig.height=10}
#| label: fig-all-fig-poetic
#| fig-cap: All plots combined for Figure 4 in the paper.
...
plots_aligned_2 <- cowplot::align_plots(
    plot_list$is_responder,
    plot_list$plot_patients,
    align = "none", 
    axis = "l",
    #align = "v",
    greedy = TRUE
