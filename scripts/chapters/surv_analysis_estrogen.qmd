...
name_document <- "surv_analysis_estrogen"
sapply(
    paste0(
        "../../results/", c("plots", "rds_files", "tables"),
        "/", name_document
    ),
    dir.create,
    showWarnings = FALSE,
...
selected_data %>% 
    ...
    ggplot2::scale_color_manual(
        values = get_colors_pam50(selected_data),
        labels = c(
...
tcga_coldata <- read.csv(
    "../../data/BRCA.clin.merged.txt", 
    sep = "\t"
) %>% 
    t 
colnames(tcga_coldata) <- tcga_coldata[1, ]
tcga_coldata <- data.frame(tcga_coldata)
tcga_coldata <- tcga_coldata %>%
    dplyr::rename(
        paper_patient = patient.bcr_patient_barcode,
        ER.IHC = patient.er_level_cell_percentage_category
    ) %>%
    dplyr::mutate(paper_patient = toupper(paper_patient))

tcga_coldata <- tcga_coldata[2:nrow(tcga_coldata), ]
rownames(tcga_coldata) <- NULL
tcga_coldata <- dplyr::inner_join(
    tcga_coldata, 
    datasets$tcga %>%
        colData %>%
        data.frame,
...
scanb_samples_her2_neg <- colData(datasets$scanb) %>% data.frame %>%
    dplyr::filter(
        node_stage != "NX" &
        !(tumor_stage %in% c("T4", "Tis", "TX")) & 
        er_status == "pos" & 
...
metabric_samples_her2_neg <- colData(datasets$metabric) %>% data.frame %>%
    dplyr::filter(
        CHEMOTHERAPY == "NO" &
        er_status == "pos" & 
        HORMONE_THERAPY == "YES" & 
...
nb_pts_events <- data.frame(
    cohort = c(
        "TCGA", "TCGA High ER", "SCANB", "METABRIC", 
    ...
    nb_events = c(
        datasets$tcga %>% colData %>% data.frame %>%
            ...
            dplyr::pull(n),
        datasets$tcga %>% colData %>% data.frame %>%
            dplyr::filter(sample_name %in% tcga_high_er & os_status == 2) %>%
            dplyr::summarise(n = n()) %>%
            dplyr::pull(n),
...
data.frame(
    ...
    her2_positive = c(
       datasets$scanb %>% colData %>% data.frame %>%
            dplyr::filter(
                sample_name %in% scanb_samples & HER2 == "Positive"
            ) %>%
            dplyr::summarise(n = n()) %>%
            dplyr::pull(n),
       datasets$metabric %>% colData %>% data.frame %>%
...
formulas_survival <- c(
    ...
    "os_scanb_her2_neg" = "Surv(os_months, os_status) ~ age + node_stage + tumor_stage",
    ...
    "os_metabric_her2_neg" = "Surv(os_months, os_status) ~ age + npi",
    "os_metabric_high_esr1" = "Surv(os_months, os_status) ~ age + npi",
    "rfs_metabric" = "Surv(rfs_months, rfs_status) ~ age + npi",
    "rfs_metabric_her2_neg" = "Surv(rfs_months, rfs_status) ~ age + npi",
    "rfs_scanb" = "Surv(rfs_months, rfs_status) ~ age + node_stage + tumor_stage",
    "rfs_scanb_her2_neg" = "Surv(rfs_months, rfs_status) ~ age + node_stage + tumor_stage",
...
survival_results <- sapply(
    ...
    datasets = mapply(
        ...
        dataset = c(
            ...
            scanb_her2_neg = datasets$scanb,
            metabric_her2_neg = datasets$metabric
        ),
        patients = list(
            tcga_samples, 
            scanb_samples, 
            ...
            scanb_high_esr1,
            metabric_high_esr1,
            scanb_samples_her2_neg,
            metabric_samples_her2_neg
        ),
        USE.NAMES = TRUE,
        SIMPLIFY = FALSE
    ),
...
#| fig-cap: Hazard ratios for estrogen early and SET ER/PR for all the
#|      three cohorts of only endocrine treated patients with ER+ HER2- BC. 
#| label: fig-surv-all-cohorts-her2-neg
terms_to_plot <- c(
    "HALLMARK_ESTROGEN_RESPONSE_EARLY" = "Estrogen Early", 
    "SET_ERPR" = "SET ER/PR"
)
...
tables_survival %>%
    dplyr::filter(
        term %in% names(terms_to_plot) & 
            cohort %in% c("scanb_her2_neg", "metabric_her2_neg")
    ) %>% 
    dplyr::mutate(
        cohort = toupper(cohort),
        type_analysis = toupper(type_analysis),
    ...
    ggplot2::labs(
        ...
        y = "",
        caption = paste0(
            "Patients selected for survival analysis: SCAN-B ET only, HER2-; METABRIC ET only, HER2-.\n",
            "OS: Overall survival, RFS: Recurrence free survival"
        )
    ) +
    ggplot2::facet_grid(type_analysis ~ term, scales = "free_y") +
...
Below we show the correlation between estrogen response early and
SET ER/PR using the ER+ HER2- BC samples.

```{r}
cor.test(
    colData(datasets$scanb)[
        scanb_samples_her2_neg, 
        "HALLMARK_ESTROGEN_RESPONSE_EARLY"
    ],
    colData(datasets$scanb)[
...
In general they are positively correlated but not to the extreme.
...
```{r, fig.width=6, fig.height=10}
#| label: fig-scores-high-er-tcga
#| fig-cap: Scores for high ER percentage breast cancer patients.
df_scores_tcga_high_er <- datasets$tcga %>%
    colData %>%
    data.frame %>%
    dplyr::filter(sample_name %in% tcga_high_er)

df_scores_tcga_high_er %>%
    tidyr::pivot_longer(
        cols = c(HALLMARK_ESTROGEN_RESPONSE_EARLY, SET_ERPR),
        names_to = "pathway",
        values_to = "score"
    ...
    ggplot2::labs(
        ...
        subtitle = paste0(
            nrow(df_scores_tcga_high_er), " samples from TCGA"
        ),
        y = "Count"
    ) + 
    ggplot2::facet_wrap(
...
plot_fit_km$plot <- plot_fit_km$plot +
    ggplot2::labs(title = "Estrogen early, ER+ BC, Endo only") + 
    ggplot2::scale_color_hue(labels = names(labels_er_ihc)) + 
    ggplot2::scale_linetype_discrete(guide = "none") +
    ggplot2::annotate(
        "richtext", 
...
strata_levels <- levels(plot_fit_km$table$data$strata)

plot_fit_km$table <- plot_fit_km$table +
    ggplot2::scale_y_discrete(
        breaks = names(labels_er_ihc) %>%
            `names<-`(strata_levels)
    ) +
    ggplot2::scale_color_hue(labels = names(labels_er_ihc)) +
...
And including the number of events.
