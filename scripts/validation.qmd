...
pdx_only_embedding <- function(pdx_name, pdx_df_pca, treatments_pdx){
...
plot_quantification(
    staining_pdxs %>%
        #dplyr::filter(pdx %in% unique(pdx_df_pca_sub$pdx)) %>%
...
lapply(plot_estimates_all, \(x) x$results_differences) %>%
    ...
    ggplot2::labs(
        ...
        fill = "PDX",
        title = paste0(
            "Distributions of the effect sizes. P4 vs CTRL."
        )
    ) +
