...
#|     (D) colored by the $SET_{ER/PR}$ signature
size <- 2
base_size <- 18
...
plots_with_scanb$SET_ERPR <- plots_with_scanb$SET_ERPR +
    ggplot2::labs(
        color = expression(SET[ER/PR])
    ) + 
...
fuzziness_scores %>% tidyr::pivot_longer(
    ...
    ggplot2::theme(legend.position = c(0.88, 0.8))
```
...
fuzziness_scores_random %>% 
    ...
    ggplot2::labs(
        ...
        subtitle = "Only PC2 fuzziness scores",
        fill = "# missing\ngenes"
    ) +
    ggplot2::theme_bw()
```
...
available there. We select randomly a number of top genes from PC2 and PC3
to compose the list of missing genes. Again we vary the proportion of these 
genes.
...
plot_pca_random_genes(pca_random_genes_patients, patient = patient_name)
```
...
smc_df_pca <- get_pca_coordinates(smc_normalized, pca_fit) %>%
    dplyr::bind_cols(
        ...
        colData(smc_normalized) %>% data.frame %>% 
            dplyr::mutate(
                er_status = ifelse(str_detect(IHC_SUBTYPE, "ER\\+"), "pos", "neg")
            ) %>% 
...
As previously, @fig-pca-smc-er-pam50 shows the biplot of PC2 and PC3, 
...
get_plot_smc <- function(smc_df_pca, color, x = "PC2", y = "PC3"){
    
    smc_df_pca %>% 
        dplyr::filter(pam50 %in% c("basal", "her2", "lumb", "luma", "normal")) %>%
        ggplot2::ggplot(
