...
saveRDS(gsva_scores, "../results/rds_files/surv_analysis_estrogen/gsva_scores.rds")
```
...
plot_scores[[which_dataset]] <- mapply(
    ...
    which_score = rownames(gsva_scores[[which_dataset]]),
    title = rownames(gsva_scores[[which_dataset]]),
    MoreArgs = list(
...
survival_results <- sapply(
    ...
    name_scores = rownames(gsva_scores[[1]]),
...
The plot below shows the forest plot of $SET_{ER/PR}$ for all 
three cohorts. Notice how small the hazard ratio is in all cases. The 
variability changes depending on the cohort, specially because they have
different follow-up times, SCANB being the shortest. 
...
plots_seterpr <- lapply(
    forest_plots$os, 
    function(x){
        patchwork::wrap_elements(ggplotify::grid2grob(print(x[[which_pathway]])))
    } 
)

plots_seterpr$tcga / plots_seterpr$scanb / plots_seterpr$metabric
```
