...
# we also plot ESR1 levels along here
esr1_levels <- log(assay(datasets$scanb)["ESR1", ], base = 2)
...
nb_pts_events <- data.frame(
    cohort = c("TCGA", "SCANB", "METABRIC", "SCANB High ER"),
    number_patients = c(
        length(tcga_samples),
...
survival_results <- sapply(
    ...
    datasets = mapply(
        ...
        dataset = c(datasets, scanb_high_er = datasets$scanb),
        patients = list(
            tcga_samples, 
            scanb_samples, 
            metabric_samples, 
...
```{r}
tables_survival %>% 
    dplyr::filter(cohort != "scanb_high_er") %>%
