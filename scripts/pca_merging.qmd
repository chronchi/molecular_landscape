...
The first two components correspond to the batch effect. The second component
explains the ER status and the two cohorts are well inter mingled. When plotting
the second and third components and coloring by the molecular subtype, we see a
...
```{r}
#| eval: false
df_pca_coordinates_og <- datasets_pca_coordinates %>% 
    do.call(rbind, .) %>%
    data.frame %>% 
...
saveRDS(
    df_pca_coordinates_og,
    "../results/rds_files/pca_merging/df_pca_coordinates_og.rds"
)
```

```{r}
#| label: fig-pca-og
#| fig-cap: Original PC2 and PC3 of all samples, including test and 
#|     train samples, of TCGA and METABRIC.
plot_pca_coordinates(
    df_pca_coordinates_og %>% dplyr::filter(cohort %in% c("tcga", "metabric")),
    color = "cohort",
    x = "PC2",
    y = "PC3",
    size = 1,
...
```{r}
#| eval: false
corrected_pca_coordinates <- datasets_pca_coordinates
corrected_pca_coordinates$metabric[, "PC2"] <- 
    corrected_pca_coordinates$metabric[, "PC2"] + 0.5
corrected_pca_coordinates$metabric[, "PC3"] <- 
    corrected_pca_coordinates$metabric[, "PC3"] + 1

df_pca_coordinates <- corrected_pca_coordinates %>% 
    do.call(rbind, .) %>%
    data.frame %>% 
    tibble::rownames_to_column(var = "sample_name") %>%
    dplyr::inner_join(
...
```{r}
#| label: fig-pca-corrected
#| fig-cap: Corrected PC2 and PC3 of all samples, including test and 
#|     train samples, of TCGA and METABRIC. Only METABRIC samples were
#|     corrected
...
By summing the offset is corrected, as shown in @fig-pca-corrected. 
The question that remains still is what is
causing this offset. Could it be the expression of some genes or it is an offset
inherent by the averaging of house keeping genes?
...
```{r}
#| label: fig-pca-age
#| fig-cap: Corrected PC2 and PC3 of samples from TCGA and METABRIC
#|     colored by age.
plot_pca_coordinates(
    df_pca_coordinates %>% 
        dplyr::filter(cohort %in% c("tcga", "metabric")),
...
```{r}
#| label: fig-pca-npi
#| fig-cap: Corrected PC2 and PC3 of samples from METABRIC only
#|     colored by NPI.
plot_pca_coordinates(
    df_pca_coordinates %>% 
        dplyr::filter(cohort %in% c("tcga", "metabric")),
    ...
    base_size = 10,
    title = paste0(
        "Embedding of all samples from corrected METABRIC"
    )
...
#| fig-cap: Corrected PC2 and PC3 of samples from TCGA only
...
And finally @fig-pca-pam50 below shows the color by PAM50 molecular
subtype.
...
plot_pca_coordinates(
    ...
    ggplot2::labs(color = expression(SET[ER/PR]))
```

```{r}
#| label: fig-pca-early
#| fig-cap: Corrected PC2 and PC3 of samples from TCGA and METABRIC
#|     colored by the hallmark estrogen response early score.
plot_pca_coordinates(
    df_pca_coordinates %>% 
        dplyr::filter(cohort %in% c("tcga", "metabric")),
...
gradient going from the right to the left and top to bottom, showing again that
patients on the far top right probably are those patients with better prognosis.

This can actually be validate using survival analysis for the patients by using
a combination of PC2 and PC3. In this case the two components were multiplied by
each other. Since PC3 is just positive, that means a negative value is in the
region of the ER negative patients.

::: panel-tabset
### TCGA OS

```{r}
fun_stat <- function(x, y){prod(x, y)}
...
df_pca_coordinates %>% 
    ...
    dplyr::rowwise() %>%
    dplyr::mutate(pc2_pc3 = fun_stat(PC2, PC3)) %>%
    dplyr::ungroup() %>%
    survival::coxph(
        Surv(os_months, os_status) ~ 
            pc2_pc3 + age + tumor_stage + node_stage,
        data = .
    ) %>% broom::tidy() %>%
...
The score depending on PC2 and PC3 has a hazard ratio smaller than 1, though it
is very close. Regardless, it seems that in all cases the score has a small
p-value, indicating moderate variability.
...
plot_pcas_together <- function(
    ...
    a <- list(
        "og_pca" = test_og_pca[random_samples, c("PC2", "PC3")] %>% 
            data.frame %>%
            tibble::rownames_to_column(var = "sample_name"),
        "new_pca" = new_pca_results[random_samples, c("PC2", "PC3")] %>%
            data.frame %>%
            tibble::rownames_to_column(var = "sample_name")
    ) %>% dplyr::bind_rows(
        .id = "type_data"
...
```{r pca-no-norm, fig.width=20, fig.height=16}
...
concentrate close to the center, they are not so well mixed. This is to be
expected, as they have different scales, specially that the minimum value for
METABRIC is around 4.5.
...
```{r pca-norm-no-norm, fig.width=10, fig.height=4}
#| label: fig-pca-norm-no-norm
#| fig-cap: PC2 and PC3 from all METABRIC and TCGA samples. (A) Embedding
