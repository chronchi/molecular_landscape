...
```{r plots-pcas-random, fig.width=10, fig.height=10}
        ...
        jaccard_index_neighbors <- parallel::mclapply(
            ...
            function(sample_name, radius, which_seed, which_cohort){
                ...
                samples_neighborhood_new <- sapply(
                    names(new_pcas),
                    function(which_seed){
                        components_sample <- new_pcas[[which_seed]]$test[[which_cohort]][
                            sample_name, 
                            c("PC3", "PC4")
                        ] 
                        samples_neighborhood_new <- get_samples_neighborhood(
                            ...
                            sample_name,
                            radius = radius,
                            df_pca = new_pcas[[which_seed]]$test[[which_cohort]] %>%
                                data.frame %>%
                                tibble::rownames_to_column(var = "sample_name")
                        )
...
variable and can change the sign (type S error). Below is the
