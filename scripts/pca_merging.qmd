...
genes_for_pca <- most_variable_genes$symbol
...
#color <- "HALLMARK_G2M_CHECKPOINT"
#color <- "HALLMARK_TGF_BETA_SIGNALING"
    ...
    function(fits, type_analysis){
        mapply(
            function(cohort_fits, name_cohort, type_analysis){
                ...
                plot_combined_scores(tidy_survival, range_hr = 3.5) + 
                    ggplot2::labs(
                        title = paste0(
...
We see that G2M checkpoint and PI3K AKT MTOR signaling all have
hazard ratios higher than 1. The other scores are a bit more
variable and can change the sign (type S error). The 
magnitude of the HRs for G2M and PI3K seem to be concordant
across the different datasets, on TCGA the score is more variable,
probably due to the number of events available, still the sign and 
the magnitude make sense in this case.
