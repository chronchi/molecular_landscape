...
also use the survival scale to talk about the probability of having the 
recurrence event.

```{r}
quartiles_risk_score <- quantile(
    endo_only_scanb$risk_score, 
    probs = c(0, .25, .5, .75, 1)
)
...
plot(standardized_survival_curves_contrast, ci = TRUE, contrast = TRUE) +
    ...
    ggplot2::scale_y_continuous(breaks = seq(0, -0.3, by = -0.05)) + 
    ggplot2::labs(
        x = "Time since diagnosis (years)",
        y = "Difference in RFS probability",
        color = "Difference of\nrisk category",
        fill = "Difference of\nrisk category"
    ) + 
    ggplot2::theme_bw(base_size = 15) +
...
as high as 20%, a significant increase. On the other hand the differences
between intermediate and low stay around 3 to 4% after 5 years.
